...
//        graph.connect(1,4,0);
//        graph.connect(1,5,0);
        //removing edge that doesn't exists
        graph.removeEdge(1,2);
        assertEquals(0,graph.edgeSize());
        //connecting nodes thats already connected
        assertNull(graph.removeNode(0));
        graph.connect(1,2,0);
        graph.connect(1,2,0);
        assertEquals(1,graph.edgeSize());
        //connect node with him self
        graph.connect(1,1,0);
        assertEquals(1,graph.edgeSize());
        //adding node with existing key
        graph.addNode(1);
    ...
    @Test
    void Test3() {
        //checks if the graph removal works
        weighted_graph graph = new WGraph_DS();
        graph.addNode(1);
        graph.removeNode(1);
        assertEquals(0, graph.nodeSize());
        //check if its possible to connect between node that doesn't exist
        graph.addNode(1);
        graph.connect(1, 2, 0);
    ...
    void Test4(){
        ...
        graph1.addNode(4);
        graph1.addNode(5);
        graph1.connect(1,2,0);
        graph1.connect(1,3,0);
        graph1.connect(1,4,0);
        graph1.connect(1,5,0);
        graph1.removeNode(3);
        //checks the MC
        assertEquals(11,graph1.getMC());
        //asking for a node doesn't exist
        assertEquals(-1,graph1.getEdge(0,1));
    }
